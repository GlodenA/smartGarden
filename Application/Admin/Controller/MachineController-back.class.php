...
//        $machineInfo = M('Machine')->where(array("machine_id"=>$machineId,"is_delete"=>0))->find();
        $searchTime = I("searchTime");
        $timeStart = strtotime($searchTime);
        $timeEnd = $timeStart+24*60*60;
        $where["add_time"] = array('between',array($timeStart,$timeEnd));
        $where["machine_id"] = $machineId;
        $listOne = M("Site_log")->where($where)->where("type=1")->order("server_utc_date asc")->select();
        if($listOne){
            foreach($listOne as $k=>$v){
                $listOne[$k]['add_time'] = date('Y-m-d H:i:s',$v['add_time']);
            }
        }
        $listTwo = M("Site_log")->where($where)->where("type=2")->order("server_utc_date asc")->select();
        if($listTwo){
            foreach($listTwo as $k=>$v){
                $listTwo[$k]['add_time'] = date('Y-m-d H:i:s',$v['add_time']);
            }
        }
        if($listOne || $listTwo){
            $ret["status"] = 1;
