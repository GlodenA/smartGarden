            ...
            if($list1){
                foreach($list1 as $k => $v){
                    if($v["groupID"] == 158){
                        ...
                        //如果当前时间大于设备服务器上传信息时间则停止
//                        if((time()-strtotime($v["serverUtcDate"])) > 60){
//                            continue;
//                        }
                        //1.2设备位置上传记录
                        ...
                        $locData["data_type"] = $v["dataType"];
                        $locData["distance"] = $v["distance"];
                        $locData["speed"] = $v["speed"];
...
//                        }
                        ...
                        $siteData["add_time"] = $locData["add_time"];
                        $siteData["server_utc_date"] = $v["serverUtcDate"];
                        $siteData["device_utc_date"] = $v["deviceUtcDate"];
                        //8.判断设备是否绑定区域
                        ...
                        if((time()>$timeInfoAmStart && time() < $timeInfoAmEnd) || (time()>$timeInfoPmStart && time() < $timeInfoPmEnd)){
                            ...
                            if($v["isStop"] == 1){
                                ...
                                if($v["stopTimeMinute"] > $still_time){
                                    //需要静止报警，查询最新报警时间
...
//                                $whereStill["type"] = 7;
//                                $stillInfo = M("Warning_message")->where($whereStill)->order("add_time desc")->find();
//                                if($stillInfo && !$stillInfo["end_time"]){
//                                    M("Warning_message")->where(array("id"=>$stillInfo["id"]))->setField("end_time",time());
//                                }
//                            }
                        }
                        //若静止时间大于半小时，则不进行打卡操作
