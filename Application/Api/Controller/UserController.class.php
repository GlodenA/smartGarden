...
class UserController extends BaseController
{
    ...
    public function userRegister()
    {
        if (IS_POST) {
            ...
            //判断用户名是否存在
            $isIn = M('Admin')->where(array('mobile' => $data['mobile']))->getField('uid');
            if (!$isIn) {
                $this->checkSmsCode($data['mobile'],$code);
                //不存在
...
//                $data['avatar'] = $_POST['avatar'];
                ...
                if ($result) {
                    ...
                    $groupAccessData['group_id'] = 2;
                    M('Auth_group_access')->add($groupAccessData);
                    //数据返回
                    $where["uid"] = $result;
                    $userData = M('Admin')->where($where)->field("password,encrypt", true)->find();
                    parent::httpResponse(1, '注册成功', $userData);
                } else {
                    parent::httpResponse(0, '注册失败');
                }
            } else {
                parent::httpResponse(0, '用户名已经存在');
            }
        }
    }
        ...
        if (IS_POST) {
            ...
            if (!$info) {
                ...
                if ($password == $info['password']) {
                    ...
                    parent::httpResponse(1, '登录成功', $infoData);
                } else {
                    parent::httpResponse(0, '密码错误');
                }
            }
